...
public class QuerySystem : QueryCatalog
{
    ...
    internal override QueryResult lookup(CatalogManager manager, EntryCatalog catalog)
    {
        ...
        foreach (var raw in catalog.Entries(Entry.Entry.EntryType.RewriteSystem))
        {
        ...
        if (rewrite != null)
        {
            Uri resolved = UriUtils.Resolve(rewrite.RewritePrefix, compareSystem[rewriteStart!.Length..]);
        ...
        foreach (var raw in catalog.Entries(Entry.Entry.EntryType.SystemSuffix))
        {
        ...
        foreach (var raw in catalog.Entries(Entry.Entry.EntryType.DelegateSystem))
        {
        ...
        if ((bool) (manager.GetResolverConfiguration().GetFeature(ResolverFeature.URI_FOR_SYSTEM)??false))
        {
