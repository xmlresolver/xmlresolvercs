...
public class EntryUri : EntryResource {
    ...
    public EntryUri(Uri baseUri, string? id, string name, string uri, string? nature, string? purpose): base(baseUri, id, uri)
    {
        if (name.StartsWith("classpath:/")) {
            // classpath:/path/to/thing is the same as classpath:path/to/thing
            // normalize without the leading slash.
            Name = "classpath:" + name[11..];
        } else {
            Name = name;
        }
    ...
    {
        string str = $"uri {Name} {Rarr} {ResourceUri}";
