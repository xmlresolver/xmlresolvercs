...
namespace XmlResolver.Adapters;

public class SystemResolver(XmlResolver resolver): System.Xml.XmlResolver 
{
    /// <summary>
    ...
    public override object? GetEntity(Uri absoluteUri, string? role, Type? ofObjectToReturn)
     {
        IResourceRequest request = resolver.GetRequest(absoluteUri.ToString(), null, ResolverConstants.EXTERNAL_ENTITY_NATURE, ResolverConstants.ANY_PURPOSE);
        IResourceResponse resp = resolver.Resolve(request);
    ...
    /// </summary>
    /// <para>There's a horrible bug in System.Xml where it attempts to resolve public identifiers
    /// as if they were relative URIs. It turns them into things like this:</para>
    /// 
    /// <para><code>http://example.com/path/-//DTD//Example 1.0//EN</code></para>
    /// 
    /// <para>That's not in the catalog, of course, so we end up doing a web request for it, even
    /// though it's bound to fail.</para>
    ///
    /// <para>You might think, if we were clever, we could do a publicId lookup in the catalog
    ...
    /// if relative URIs are resolved against it, especially if it happens to be a <code>pack://</code>
    /// URI that was the result of loading a resource from an assembly.</para>
    ///
    /// <para>The best we can do is return "null" without attempting to access the resource
    /// with the broken URI.</para>
    /// 
    ...
    public override Uri ResolveUri(Uri? baseUri, string? relativeUri)
    {
        ...
        if (relativeUri != null) {
            ...
            if (pos >= 0 && (cpos < 0 || cpos > pos)) {
                var id = relativeUri.Substring(pos + 2);
