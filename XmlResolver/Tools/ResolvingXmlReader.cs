...
namespace XmlResolver.Tools {
    public class ResolvingXmlReader : XmlReader {
        ...
        private bool _parseStarted;
        private bool _allowXmlCatalogPI;
        ...
        public ResolvingXmlReader(Uri uri, XmlReaderSettings settings, XmlResolver resolver) : this(null, uri, settings, resolver) {
            // nop
        }

        public ResolvingXmlReader(Stream data, XmlReaderSettings settings, XmlResolver resolver) : this(data, null, settings, resolver) {
            // nop
        }
        ...
        public ResolvingXmlReader(Stream? data, Uri? baseUri, XmlReaderSettings? parseSettings, XmlResolver? xmlResolver) {
            ...
            Stream stream;
            if (data == null) {
                stream = ResourceAccess.GetStream(baseUri);
            }
            else {
                stream = data;
            }
        ...
        public override bool Read() {
            if (!_parseStarted) {
                _allowXmlCatalogPI = (bool) Resolver.Configuration.GetFeature(ResolverFeature.ALLOW_CATALOG_PI);
            }
            ...
            if (read) {
                if (!_seenRoot) {
                    if (_allowXmlCatalogPI && _reader.NodeType == XmlNodeType.ProcessingInstruction) {
                        ...
                        if ("oasis-xml-catalog".Equals(_reader.Name)) {
                            ...
                            if (pos >= 0) {
                                ...
                                if (pos >= 0) {
                                    ...
                                    ((XmlResolverConfiguration) Resolver.Configuration).AddCatalog(catalog.ToString());
                                }
                            }

                            if (pos < 0) {
                                logger.Debug("Invalid oasis-xml-catalog processing instruction {0}", _reader.Value);
                            }
                        }
                    }
                    
                    _seenRoot = _reader.NodeType == XmlNodeType.Element;
                }
            }
